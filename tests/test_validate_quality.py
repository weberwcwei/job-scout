...
class TestPlaceholderWarnings:
    ...
    def test_placeholder_target_title(self):
        ...
        diags = validate_quality(cfg)
        assert any(d.field == "profile.target_title" and d.level == "warning" for d in diags)
    ...
    def test_empty_search_locations(self):
        ...
        diags = validate_quality(cfg)
        assert any(d.field == "search.locations" and d.level == "warning" for d in diags)
...
class TestKeywordWarnings:
    def test_all_keyword_tiers_empty(self):
        cfg = _cfg(**{
            "profile.keywords": {"critical": [], "strong": [], "moderate": [], "weak": []},
        })
        diags = validate_quality(cfg)
        assert any(
    ...
    def test_critical_empty_only_strong(self):
        ...
        cfg = _cfg(**{
            "profile.keywords": {"critical": [], "strong": ["distributed"], "moderate": [], "weak": []},
        })
        diags = validate_quality(cfg)
        assert any(
    ...
    def test_critical_empty_moderate_populated(self):
        ...
        cfg = _cfg(**{
            "profile.keywords": {"critical": [], "strong": [], "moderate": ["aws"], "weak": ["docker"]},
        })
        diags = validate_quality(cfg)
        assert any(
            d.field == "profile.keywords.critical"
            and "capped at 10" in d.message
            for d in diags
        )

    def test_critical_empty_weak_populated(self):
        cfg = _cfg(**{
            "profile.keywords": {"critical": [], "strong": [], "moderate": [], "weak": ["docker"]},
        })
        diags = validate_quality(cfg)
        assert any(
            d.field == "profile.keywords.critical"
...
class TestScoringWarnings:
    def test_unreachable_min_alert_score(self):
        ...
        assert any(
            ...
            and "unreachable" in d.message.lower()
            for d in diags
        )
    ...
    def test_strong_only_max_score_excludes_keywords(self):
        ...
        cfg = _cfg(**{
            "profile.keywords": {"critical": [], "strong": ["python"], "moderate": [], "weak": []},
            "profile.target_companies": {"tier1": ["Google"], "tier2": [], "tier3": []},
...
class TestDealBreakerRegexErrors:
    ...
    def test_invalid_company_pattern(self):
        ...
        diags = validate_quality(cfg)
        assert any(
            d.field == "profile.dealbreakers.company_patterns"
            and d.level == "error"
            for d in diags
        )
